...
prediction_window   = 5
hist_window         = 15 # using how much data from the past to make prediction?
...
def work(model, data_loader, optimizers, num_epochs = num_epochs, mode = 0, schedulers = None): # mode 0: train, mode 1: test, mode 2: PLOT
    ...
    for epoch in range(num_epochs):
        ...
        print(f'Epoch {epoch+1:3}/{num_epochs:3}, ' +
              ...
              f'LR: {get_current_lr(optimizers[0]):9.10f}') # +
...
def main():
    ...
    model = SimpleNN(input_size, hidden_size, num_layers, output_size, hist_window, prediction_window, dropout, device).to(device)
    if os.path.exists('last'+model_path):
        ...
        with open('training_param_log.json', 'r') as f:
            ...
            encoder_lr = saved_data['learning_rate']
    ...
    optimizer = AdamW(model.parameters(),weight_decay=1e-5, lr=encoder_lr)
    scheduler = ReduceLROnPlateau(optimizer, mode='min', factor=0.98, patience=10, threshold=0.0001)
