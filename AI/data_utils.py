...
class StockBarDataset(Dataset):
    def __init__(self, data, timestamp, pred_window, not_close_batch_norm_lst, close_idx): # assumes that first column of data is timestampstr.
        ...
        self.timestamp = timestamp[:,-pred_window-1] # timestamp of close price
        # (n) 1-d array
        # print("not_close_batch_norm_lst: ", not_close_batch_norm_lst)

        self.close_idx = close_idx
        feature_num = data.shape[2]
        x_raw = data[:,:-pred_window,:] # slicing off the last entry of input
        # print("x.shape: ",self.x.shape)
        # x.shape: (data_num, window_size, feature_num)
        y_raw = data[:,-pred_window:,close_idx] 
        tmp = x_raw[:,-1,close_idx:close_idx+1]
        self.pct_change = (y_raw - tmp)/tmp * 100 * pct_pred_multiplier 
        # don't need to normalize y; this is the best way (NO IT ISN"T!); present target as the percentage growth with repsect to last close price.

        self.x = batch_norm(x_raw, not_close_batch_norm_lst, close_idx)

        print("x[0]: ", self.x[0][0])
        print('y[0]: ', self.pct_change[0])
        
        print('timestamp: ', self.timestamp[0])
        self.price = x_raw[:,-1,close_idx] # close price

    def __len__(self):
        return self.pct_change.shape[0]

    def __getitem__(self, idx):

        return self.x[idx,:,:], self.pct_change[idx,:,None], self.price[idx], self.timestamp[idx]  #self.x_mean[idx,:], self.x_std[idx,:]
        # (hist_window, feature_num), (pred_window), (1), (1)
    def get_item(self, idx):
        return self.__getitem__(idx)

class StockBarClassificationDataset(Dataset):
    def __init__(self, data, timestamp, pred_window, not_close_batch_norm_lst, close_idx, thres): # assumes that first column of data is timestampstr.
        print('data.shape: ', data.shape)
        print('timestamp.shape: ', timestamp.shape)
        assert data.shape[0] == timestamp.shape[0], "data and timestamp must have the same number of rows."
        # print(data.shape)
        ...
        # print("x.shape: ",self.x.shape)
        # x.shape: (data_num, window_size, feature_num)
        y_raw = data[:,-pred_window:,close_idx:close_idx+1] 
        tmp = x_raw[:,-1:,close_idx:close_idx+1]
        self.pct_change = (y_raw - tmp)/tmp * 100 * pct_pred_multiplier 
        # pct_change_mean = np.mean(self.pct_change, axis=1, keepdims=False)
        self.change_type = np.where(self.pct_change < -thres, -1, np.where(self.pct_change > thres, 1, 0))
...
class StockTradeDataset(Dataset):
    pass

class MultiStockBarDataset(Dataset):
    def __init__(self, data_lst, pred_window, not_close_batch_norm_lst, close_idx):
        self.dataset_lst = []
        for data, timestamp in data_lst:
            print("data.shape: ", data.shape)
            print("timestamp.shape: ", timestamp.shape)
            self.dataset_lst.append(StockBarDataset(data, timestamp, pred_window, not_close_batch_norm_lst, close_idx))
        pass
...
def load_n_split_data(data_path, hist_window, pred_window, batch_size, train_ratio, normalize = True, test = False):
    ...
    data_prep_window = hist_window + pred_window
    ...
    if not test:
        train_dataset = StockBarDataset(train_data, train_timestamp, pred_window, not_close_batch_norm_lst, close_idx)
        val_dataset = StockBarDataset(val_data, val_timestamp, pred_window, not_close_batch_norm_lst, close_idx)
        ...
        test_dataset = StockBarDataset(data_norm, timestamp, pred_window, not_close_batch_norm_lst, close_idx)
...
def load_multi_symbol_data(data_path_lst, hist_window, pred_window, batch_size, train_ratio, close_idx, normalize = True, test = False):
    ...
    # else:
    #     data_norm = df.values
    #     test_dataset = TwoDStockBarDataset(data_norm, pred_window, close_idx)
    #     test_loader = DataLoader(test_dataset, batch_size=1, shuffle=False, num_workers=0, pin_memory=False)
    #     return test_loader, df.columns
    ...
    if not test:
        train_dataset = StockBarDataset(train_data, pred_window, close_idx)
        val_dataset = StockBarDataset(val_data, pred_window, close_idx)
        train_loader = DataLoader(train_dataset, batch_size=batch_size, shuffle=True, num_workers=0, pin_memory=False) 
        ...
        test_dataset = StockBarDataset(data_norm, pred_window, close_idx)
        test_loader = DataLoader(test_dataset, batch_size=1, shuffle=False, num_workers=0, pin_memory=False)
        print(f'data loading completed in {time.time()-start_time:.2f} seconds')
        return test_loader, df.columns
...
def main():
    tmp, tmp2 = load_n_split_data(training_data_path, hist_window, pred_window, batch_size, train_ratio = 0.1, normalize = True, test = False)
