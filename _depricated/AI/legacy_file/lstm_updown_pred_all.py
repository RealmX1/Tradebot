...
output_size         = 23     # Number of output values (closing price 1~10min from now)
prediction_window   = 5
hist_window         = 50 # using how much data from the past to make prediction?
...
def work(model, data_loader, optimizers, num_epochs = num_epochs, mode = 0, schedulers = None): # mode 0: train, mode 1: test, mode 2: PLOT
    ...
    discount_rate = 0.8
    weights = torch.pow(torch.tensor(discount_rate), torch.arange(prediction_window))
    weights = weights.repeat(feature_num, 1).transpose(0, 1).float().to(device)
    ...
    for epoch in range(num_epochs):
        ...
        for i, (x_batch, y_batch, x_raw_close) in enumerate(data_loader):
            ...
            weighted_loss = (loss * weights).mean(dim = 0)
            # print("weighted_loss.shape: ", weighted_loss.shape)
            ...
            total_cells, same_cells, total_cells_list,same_cells_list = get_direction_diff(y_batch[:,:,close_idx], y_pred[:,:,close_idx])
...
def main():
    ...
    model_path = 'lstm_updown_S2S_attention_pred_all.pt'
    print("loaded in ", time.time()-start_time, " seconds")
    
    train_loader, test_loader = load_n_split_data(data_path, hist_window, prediction_window, batch_size, train_ratio, global_normalization_list = None, pred_all=True)
