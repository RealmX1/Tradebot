    ...
    try:
        ...
        for epoch in range(num_epochs):
            ...
            for i, (x_batch, y_batch, x_raw_close, timestamp) in enumerate(data_loader):
                ...
                # if train:
                #     y_pred = model(x_batch, y_batch) # [N, prediction_window, 1(output_dim)]
                # else:
                ...
                loss = loss_fn(y_pred.squeeze(2), y_batch.squeeze(2)) # note that squeeze isn't inplace; and their shape are stil 3D.
                ...
                y_batch_safe = y_batch.clone().detach().cpu().numpy().squeeze(2)
                y_pred_safe = y_pred.clone().detach().cpu().numpy().squeeze(2)
